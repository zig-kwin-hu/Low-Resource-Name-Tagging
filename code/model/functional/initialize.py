...
def init_cnn_weight(cnn_layer, seed=777777):
    """初始化cnn层权重
    Args:
...
def init_lstm_weight(lstm, num_layer=1, seed=777777):
    """初始化lstm权重
    Args:
        lstm: torch.nn.LSTM
...
def init_linear(input_linear, seed=777777):
    """初始化全连接层权重
    """
    torch.manual_seed(seed)
...
def init_embedding(input_embedding, seed=777777):
    """初始化embedding层权重
    """
    torch.manual_seed(seed)
